						...
						<div class="row">
							<div class="col-md-8" id="tml">
								<div class="text-center" style="background: #1761a0; color: #fff; padding: 5px 0;">
									<h2 style="margin: 0; padding: 0; font-size: 25px;">Tag search: " {!! $tag !!} "</h2>
								</div>
								<div class="well wl">
											...
											@foreach($res as $rs)
												...
												<div class="well sub_infoCon">
													<div style="text-align: justify;" id="pid" data-postid="{{ $rs->id }}">
														<h5>{!! $rs->uploader !!}</h5>
														<p>{!! $rs->created_at !!}</p>
														<p>Course Code : {!! $rs->course_code !!} &nbsp;<span>{!! $tag !!}</span></p>
														<div id="qst">Q{!! $rs->id !!} . {!! str_limit($rs->question,140,'...') !!}</div>
													</div>

													<div class="col-md-12" style="padding: 0;">
														<a class="btn btn-sm btn-primary" href="single_view?qid={!! $rs->id !!}&sub={!! $rs->course_code !!}&semester_tag={!! $rs->semester !!}">View Question</a>
													</div>
